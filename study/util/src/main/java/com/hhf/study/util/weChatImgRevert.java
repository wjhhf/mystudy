...
public class weChatImgRevert {
    ...
    public static void main(String[] args) {
        String path = "D:\\wechat\\WeChat Files\\qq372573336\\FileStorage\\Image\\2019-11";
        String targetPath ="D:\\wechat\\WeChat Files\\qq372573336\\FileStorage\\Image\\convertImage";
        String[] paths=path.split("\\\\");
        targetPath+=File.separator+paths[paths.length-1];
        //convert(path, targetPath);
        String oldPath = "D:\\wechat\\WeChat Files\\qq372573336\\FileStorage\\Image\\Thumb\\2019-11\\a75f3215708c78ce61955b46581fd439_t.dat";
         convertSingleFile(oldPath,"D:\\wechat\\WeChat Files\\qq372573336\\FileStorage\\Image\\convertImage");
    }
    ...
    private static void convert(String path, String targetPath) {
        ...
        Arrays.stream(file).parallel().forEach(file1 -> {
            ...
            if("jpg".equalsIgnoreCase(xor[0].toString())){
                xor[1] = xor[1] == null ? x.get() : xor[1];
                Calendar cal = Calendar.getInstance();
                long time = file1.lastModified();
                cal.setTimeInMillis(time);
                File newFile = new File(targetPath + File.separator + new SimpleDateFormat("yyyy-MM-dd HH_mm_ss").format(cal.getTime())+"_"+file1.getName().split("\\.")[0] + (xor[0] != null ?
                        "." + xor[0] : ""));
                newFile.setLastModified(file1.lastModified());
                ...
                System.out.println("文件"+file1.getName()+"格式为"+xor[0]+",非JPG格式，无需转换");
            }
    ...
    private static void convertSingleFile(String oldPath,String newPath) {
        File oldFile =new File(oldPath);
        if(!oldFile.isFile()){
            System.out.println(oldPath+"不是文件类型");
            return;
        }
        Calendar cal = Calendar.getInstance();
        ...
        Object[] xor = getXor(oldFile);
        String newName=newFileDir+File.separator+new SimpleDateFormat("yyyy-MM-dd HH_mm_ss").format(cal.getTime())+"_"+oldFile.getName().split("\\.")[0] + (xor[0] != null ?
                "." + xor[0] : "");
        File newFile= new File(newName);
        try {
            InputStream reader = new FileInputStream(oldPath);
            ...
            System.out.println("文件"+oldPath+"转换完毕，路径为"+newFile.getAbsolutePath());
