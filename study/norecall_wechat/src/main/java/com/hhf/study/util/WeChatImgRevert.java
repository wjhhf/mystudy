...
public class WeChatImgRevert {
    private static Logger logger = LoggerFactory.getLogger(WeChatImgRevert.class);
    public static void main(String[] args) {
        ...
        String oldPath = "D:\\wechat\\WeChat Files\\qq372573336\\FileStorage\\Image\\2019-11\\0ba41d4c9a4866cf89c17bce1d0c11fd.dat";
         convertSingleFile(new File(oldPath),"D:\\convert\\backup");
    }
    ...
    public static void convert(String path, String targetPath) {
        ...
        Arrays.stream(file).parallel().forEach(file1 -> {
            ...
            try (InputStream reader = new FileInputStream(file1);
                ...
                logger.info(file1.getName() + "(大小:" + ((double) file1.length() / 1000) + "kb,异或值:" + xor[1] + ")," +
                        "进度：" + integer.get() +
                        "/" + size);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static void convertSingleFile(File oldFile,String newPath) {
        if(!oldFile.isFile()){
            logger.error(oldFile+"文件不存在");
            return;
        }
        Calendar cal = Calendar.getInstance();
        ...
        String newName=newFileDir+File.separator+new SimpleDateFormat("yyyy-MM-dd HH_mm_ss_SSS").format(cal.getTime())+ (xor[0] != null ?
                "." + xor[0] : "");
        File newFile= new File(newName);
        OutputStream writer=null;
        try {
            InputStream reader = new FileInputStream(oldFile);
            ...
            logger.info("文件"+oldFile+"转换完毕，路径为"+newFile.getAbsolutePath());
